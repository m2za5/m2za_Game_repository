...
while not intro_done:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RETURN:  # 엔터 키를 누르면 다음 이미지와 텍스트 표시
                current_index += 1
    ...
    # 현재 인덱스가 텍스트 리스트 길이보다 작을 때만 텍스트 표시
    if current_index < len(introText):
        ...
        for line in introText[current_index]:
            ...
            y_offset += text_rect.height  # 다음 라인의 위치를 조절합니다.
...
while not intro_done:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            sys.exit()
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RETURN:  # 엔터 키를 누르면 게임으로 넘어감
                intro_done = True
